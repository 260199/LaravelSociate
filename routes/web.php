...
Route::get('/notifications/detail/{id}', [LayoutController::class, 'notificationDetail'])->name('notifications.detail');
...
Route::group(['middleware' => ['auth']],function(){
    ...
    Route::group(['middleware' => ['AksesRole:1']],function(){
        ...
        Route::resource('/jekegs', \App\Http\Controllers\JenisKegiatanController::class)->middleware('auth');
        Route::get('/admin/actv/filter', [ActivityController::class, 'filter'])->name('actv.filter');

        
        Route::post('/actv/{id}/approve', [ActivityController::class, 'approve'])->name('actv.approve');
        Route::post('/actv/reject/{id}', [ActivityController::class, 'reject'])->name('actv.reject');
        ...
        Route::get('/actv/{id}/detail', [\App\Http\Controllers\DailyController::class, 'detail'])
                ->name('actv.detail')
                ->middleware('signed'); // Menambahkan middleware signed
        Route::get('/actv/{id}/info', [\App\Http\Controllers\DailyController::class, 'info'])->name('actv.info');
        ...
        Route::get('users/{userId}/activities', [\App\Http\Controllers\LayoutAdminController::class, 'activities'])->name('users.activities');
        // Route untuk detail aktivitas
        Route::get('activities/{activityId}', [\App\Http\Controllers\LayoutAdminController::class, 'activityDetail'])->name('users.activity.detail');
        ...
        // Route untuk melihat aktivitas pengguna
