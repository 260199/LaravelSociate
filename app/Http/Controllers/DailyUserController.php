...
class DailyUserController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Filter aktivitas hanya untuk user yang sedang login
        $daily = Daily::where('user_id', Auth::id())
            ->orderBy('created_at', 'desc');
    
        // Jika ada status yang dipilih oleh user
        if ($request->has('status') && $request->status != '') {
            $daily->where('status', $request->status);
        }
    
        $daily = $daily->get();
        $jekegs = jekeg::all();
    
        return view('user.actv.index')->with([
            'user' => Auth::user(),
            'notiff' => $notiff,
            'notifications' => $notif,
            'daily' => $daily,
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->hasFile('images')) {
            ...
            foreach ($request->file('images') as $index => $image) {
                ...
                ->save($imagePath, 80); // ubah jadi kualitad 80%
    ...
    private function sendEmailNotification($daily)
    {
        ...
        try {
            ...
            $body = 
            "
                <h3>Kegiatan Harian Telah Selesai - {$daily->user->name}</h3>
    ...
    public function detail($id)
    {
        ...
        return view('user.detail.index')->with([
            'user' => Auth::user(),
            'notiff' => $notiff,
            'notifications' => $notif,
            'daily' => $daily
