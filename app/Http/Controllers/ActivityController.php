...
use App\Events\DailyApprove;
use App\Events\DailyCreated;
use App\Events\DailyReject;
use App\Events\DailyReport;
use App\Events\DailyUpdated;
...
class ActivityController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $daily->update([
            ...
            'jekeg_id' => $request->jekeg_id,
            'deskripsi' => $request->deskripsi,
            'done_at' => $daily->done_at, 
        ]);
    
        // broadcast(new DailyUpdated($daily));
   ...
   private function sendEmailNotification($daily)
{
        ...
        foreach ($dailies as $daily) {
            if ($daily->status != 'diterima' && $daily->filedailies()->exists()) {
                ...
                $daily->notifications()->create([
                    'type' => 'Laporan',
                    'message_admin' => ' Kamu telah menerima pelaporan aktivitas',
    ...
    public function approve($id)
    {
        ...
        if ($daily->status === 'dilaporkan') {
            ...
            broadcast(new DailyApprove($daily));
    ...
    public function reject(Request $request, $id)
    {
        ...
        if ($daily->status === 'dilaporkan') {
            ...
            $daily->status = 'ditolak'; 
            $daily->reject_message = $request->reject_message;
            $daily->save();

            $daily->notifications()->create([
                'type' => 'Laporan Aktivitas Ditolak!',
                'message' => $daily->user->name . ' laporan kamu ditolak: ' . $request->reject_message,
                'message_admin' => 'Aktivitas ' . $daily->user->name . ' Telah Di Tolak!',
                'user_id' => $daily->user_id,
                'read' => false,
            ]);
            broadcast(new DailyReject($daily));
            return redirect()->back()->with('success', 'Kegiatan berhasil ditolak.');
        }

        return redirect()->back()->with('error', 'Kegiatan tidak valid untuk ditolak.');
    }
}